...
* Collecting, storing and querying:
    * All these will be collected on TiKV LockManager, and will need a ne`w RPC entry for TiDB to query. LockManager won't store the un-hashed key or SQL_DIGEST for now, so we need to modify it.
