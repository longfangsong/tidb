...
| `TYPE`     | `enum('Record', 'Index', 'Other')` | Type of this key |
| `TABLE_NAME`     | `varchar` | Which table is the `KEY` in |
| `ID`     | `unsigned bigint` | RowID when `Type` is `Record`, IndexID when `Type` is `Index` |
